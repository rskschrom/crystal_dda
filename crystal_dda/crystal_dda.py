...
def subset_branched_dda(a, amax, xp_br, yp_br, afrac, numxp=150, numzp=9):
    ...
    header = "#generated by crystal_dda.py"+\
             "\n#shape: 'read'"+\
             f"\n#box size: {delta_x:.0f}x{delta_y:.0f}x{numzp:.0f}\n"
    np.savetxt(fname, np.c_[xdip, ydip, zdip],
...
def plate_dda(a, numxp, numzp):
    ...
    header = "#generated by crystal_dda.py"+\
             ...
             f"\n#box size: {delta_x:.0f}x{delta_y:.0f}x{numzp:.0f}"
    np.savetxt(fname, np.c_[xdip, ydip, zdip],
               fmt=('%0.f','%0.f','%0.f'), header=header, comments='')
